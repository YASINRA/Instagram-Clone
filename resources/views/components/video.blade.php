...
<div x-data="{ playing: false, muted: false }" @click.outside="$refs.player.pause()" x-intersect:leave="$refs.player.pause()"
    class=" m-auto h-full w-full relative">
    ...
    @if ($controls == true)
        ...
        <div class="absolute z-[1000] bottom-2 right-2 m-4 bg-gray-900 text-white  rounded-lg p-1   cursor-pointer">
            ...
            <!-- Mute   -->
            <svg x-cloak x-show="!muted" @click="$refs.player.muted = true; muted = true" class="w-4 h-4"
            ...
            <!-- Un Mute -->
            <svg x-show="muted" @click="$refs.player.muted = false; muted = false" class="w-4 h-4"
                xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                <path
